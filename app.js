...
window.getDynamicSettings = function() { 
    return new Promise((resolve) => { 
        ...
        db.transaction(["settings"], "readonly").objectStore("settings").getAll().onsuccess = (e) => { 
            if (e.target.result) { e.target.result.forEach(s => { settings[s.key] = s.value; }); } resolve(settings); 
        }; 
    });
};
...
window.buildEscPosReceipt = async function(orderId, order, deposit, remaining, payMethod, newPoints, newFree) { 
    ...
    const CMD_INIT = "\x1B\x40"; const CMD_CENTER = "\x1B\x61\x01"; const CMD_LEFT = "\x1B\x61\x00"; const CMD_BOLD_ON = "\x1B\x45\x01"; const CMD_BOLD_OFF = "\x1B\x45\x00";
...
// ==========================================
// 3. CORE LOGIN & WORKSPACE (ASLI DARI DOCX)
// ==========================================
    ...
    if (isMenuLocked) { 
        ...
        if (lotteryBtn) { 
            if (activeCustomerProfile && (activeCustomerProfile.lastClaimDate === todayStr || activeCustomerProfile.isNoWA)) {
                lotteryBtn.disabled = true; lotteryBtn.innerText = "🎫 Sudah Klaim Hari Ini"; lotteryBtn.title = "Pelanggan ini sudah mengambil jatah undian untuk hari ini.";
            } else { 
                lotteryBtn.disabled = false; lotteryBtn.innerText = "🎫 Pilih Undian"; lotteryBtn.title = ""; 
            } 
        } 
...
window.proceedToUnlock = function(phone, name) { 
...
window.unlockMenu = function(isGuest) { 
    ...
    if (isGuest) { 
        ...
        window.proceedToUnlock(phone, name);
    } else { 
...
// FIX 1: AUTOCOMPLETE DIJAMIN MUNCUL SAAT KLIK (ONMOUSEDOWN KE SELECT)
    ...
    db.transaction(["members"], "readonly").objectStore("members").getAll().onsuccess = (ev) => {
        ...
        if (matches.length > 0) {
            resBox.innerHTML = matches.map(m => `
                <div class="autocomplete-item" onmousedown="window.selectMember('${m.phone}')" style="padding: 12px 15px; border-bottom: 1px solid #eef2f3; cursor: pointer; text-align: left; background: #fff; font-size: 15px;">
...
window.submitEditMember = function() { 
    ...
    db.transaction(["members"], "readonly").objectStore("members").get(oldPhone).onsuccess = (e) => { 
        ...
        antreans.forEach(a => { 
            if (a.profile && a.profile.phone === oldPhone) { a.profile.phone = newPhone; a.phoneInput = newPhone; } 
        }); 
...
window.openLotteryModal = function() { 
    ...
    window.globalPromos.forEach(p => { 
        if(p.weeklyQuota === 0 || p.usedQuota < p.weeklyQuota) { 
...
window.syncMasterData = async function(forceAwait = false) { 
    ...
    try { 
        ...
        if (result.status === "Success") {
            ...
            // PRIORITY 1: PIN KASIR DAN MENU (Untuk login Instan)
            let p1 = new Promise((resolve) => {
                const tx = db.transaction(["staff", "menu"], "readwrite");
                const staffStore = tx.objectStore("staff"); 
                staffStore.clear(); result.data.staff.forEach(s => staffStore.add(s)); 
                const menuStore = tx.objectStore("menu");
                menuStore.clear(); result.data.menu.forEach(m => menuStore.add(m)); 
                
                tx.oncomplete = () => {
                    globalMenuData = result.data.menu;
                    if (!document.getElementById("pos-screen").classList.contains("hidden")) { window.loadMenuUI(); window.renderActiveTickets(); }
                    resolve();
                };
            });
            if(forceAwait) await p1;

            // PRIORITY 2: MEMBER DAN LAINNYA
            const tx2 = db.transaction(["settings", "members", "expense_categories"], "readwrite");
            
            const memStore = tx2.objectStore("members"); 
            memStore.clear(); result.data.members.forEach(m => memStore.add(m)); 
            
            const expCatStore = tx2.objectStore("expense_categories"); 
            expCatStore.clear(); 
            if(result.data.expenseCategories) result.data.expenseCategories.forEach(c => expCatStore.add({name: c}));
            
            const settingsStore = tx2.objectStore("settings"); 
            settingsStore.clear(); 
...
window.renderProductGrid = function() { 
    ...
    globalMenuData.filter(i => i.category === currentCategory).forEach(item => { 
        ...
        card.onclick = () => { if(isMenuLocked) return; if (item.inputMode === "DECIMAL") window.openNumpad(item); else window.addToCart(item, 1); }; 
        grid.appendChild(card);
    }); 
};

window.openNumpad = function(item) { 
    activeNumpadItem = item; numpadValue = "0"; 
...
window.numpadPress = function(val) { 
    if (val === 'DEL') { 
        ...
        numpadValue = numpadValue === "0" ? String(val) : numpadValue + val; 
    }
    document.getElementById("numpad-display").innerText = numpadValue; 
};

window.confirmNumpad = function() { 
    let qty = parseFloat(numpadValue); 
    if (qty > 0) window.addToCart(activeNumpadItem, qty); 
    window.closeNumpad();
};
...
// FIX 3: UKURAN TOMBOL CART (+/-) LEBIH BESAR
...
// FIX 2: POINT CALCULATION (LOYALTY PARADOX)
window.openReview = function() { 
    ...
    let pf = document.getElementById("pay-free"); if(pf) pf.value = 0; 
    ...
    if (promoContainer) { 
        ...
        if (promoHtml) promoContainer.classList.remove("hidden"); else promoContainer.classList.add("hidden"); 
    } 
...
// FIX 5: EXPOSE CLOSE REVIEW TO WINDOW
window.closeReview = function() { document.getElementById("review-modal").classList.add("hidden"); };
window.closeReviewModal = window.closeReview; window.cancelOrder = window.closeReview; window.reviewOrder = window.openReview;
...
window.calculateRemaining = function() { 
    ...
    const totalAccounted = c + q + t + hp + tp + f; 
...
window.finalizeOrder = async function(shouldPrint) { 
    ...
    const qris = Number(document.getElementById("pay-qris").value) || 0; const transfer = Number(document.getElementById("pay-transfer").value) || 0; 
    const hotelPiutang = Number(document.getElementById("pay-hotel-piutang")?.value) || 0; const tamuPiutang = Number(document.getElementById("pay-tamu-piutang")?.value) || 0;
    ...
    if (custPhone !== "-") { 
        ...
        let totalPoints = initialPoints + coinsEarned;
        ...
        redeemedList.forEach(rp => { 
            if(rp.source === 'stored' && activeCustomerProfile.storedRewards[rp.item]) activeCustomerProfile.storedRewards[rp.item] -= rp.qty; 
        }); 
    ...
    if (internalCoins > 0) { 
        txMenu.openCursor().onsuccess = (ev) => { 
            ...
            if (cursor) { 
                if (String(cursor.value.name).toLowerCase() === "koin_fisik") { const updated = cursor.value; updated.currentStock = Math.max(0, updated.currentStock - internalCoins); cursor.update(updated); } cursor.continue(); 
            } 
        };
    } 
    ...
    if (shouldPrint) { 
        await window.buildEscPosReceipt(orderPayload.orderId, orderPayload, (cash + qris + transfer + free + totalPiutang), 0, payString, newPoints, newFree);
    }
...
window.renderActiveTickets = function() { 
    ...
    activeLaundryTickets.forEach((ticket) => { 
        ...
        const remaining = ticket.grandTotal - totalPaid; let receiptText = ticket.readableReceipt || "";
    ...
    if (type === 'orders') { 
        db.transaction(["orders"], "readonly").objectStore("orders").getAll().onsuccess = (e) => { 
            ...
            shiftOrders.forEach(o => { 
                ...
                let detailBtn = `<button onclick="window.viewOrderDetails('${o.orderId}')" style="background:#f39c12; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;" title="Lihat Detail Rincian">👁️ Detail</button>`; 
    ...
    db.transaction(["local_shift_history"], "readonly").objectStore("local_shift_history").get(shiftId).onsuccess = (e) => { 
        ...
        if(s.foodSummary) { 
            for(const [name, qty] of Object.entries(s.foodSummary)) { 
                let qd = (qty % 1 !== 0) ? Number(qty).toFixed(2) : qty;
                foodHtml += `<div style="display:flex; justify-content:space-between; border-bottom:1px dashed #eee; padding:4px 0;"><span>${name}</span> <strong>${qd}x</strong></div>`;
            }
        } 
        ...
        if(document.getElementById("sd-hotel-piutang")) document.getElementById("sd-hotel-piutang").innerText = `Rp ${(s.totalHotelPiutang||0).toLocaleString('id-ID')}`;
        if(document.getElementById("sd-tamu-piutang")) document.getElementById("sd-tamu-piutang").innerText = `Rp ${(s.totalTamuPiutang||0).toLocaleString('id-ID')}`;
        document.getElementById("sd-expenses").innerText = `Rp ${(s.totalExpenses||0).toLocaleString('id-ID')}`;
        
        let freeItems = 0; let nomDiskon = s.totalFree || 0;
        if (document.getElementById("sd-free-items")) document.getElementById("sd-free-items").innerText = (s.totalFreeItems || 0) + " Item";
        if (document.getElementById("sd-discount-nom")) document.getElementById("sd-discount-nom").innerText = "Rp " + nomDiskon.toLocaleString('id-ID');
...
// FIX 6: Shift report print & trigger end shift exposed
...
window.triggerEndShift = async function() {
    ...
    const shiftPayload = {
        ...
        totalFreeItems: data.totalFreeItems, totalDiscountNominal: data.totalDiscountNominal,
...
window.openShiftReport = function() {
    ...
    tx.oncomplete = () => {
        ...
        document.getElementById("sd-id").innerText = window.currentShiftData.shiftId; 
        document.getElementById("sd-login").innerText = formatTimeOnlyWIB(window.currentShiftData.loginTime); 
        document.getElementById("sd-logout").innerText = "Saat Ini"; 
        document.getElementById("sd-omset").innerText = `Rp ${(window.currentShiftData.totalOmset||0).toLocaleString('id-ID')}`; 
        document.getElementById("sd-cash").innerText = `Rp ${(window.currentShiftData.totalCash||0).toLocaleString('id-ID')}`; 
        document.getElementById("sd-qris").innerText = `Rp ${(window.currentShiftData.totalQris||0).toLocaleString('id-ID')}`;
        document.getElementById("sd-transfer").innerText = `Rp ${(window.currentShiftData.totalTransfer||0).toLocaleString('id-ID')}`; 
        if(document.getElementById("sd-hotel-piutang")) document.getElementById("sd-hotel-piutang").innerText = `Rp ${(window.currentShiftData.totalHotelPiutang||0).toLocaleString('id-ID')}`;
        if(document.getElementById("sd-tamu-piutang")) document.getElementById("sd-tamu-piutang").innerText = `Rp ${(window.currentShiftData.totalTamuPiutang||0).toLocaleString('id-ID')}`;
        document.getElementById("sd-expenses").innerText = `Rp ${(window.currentShiftData.totalExpenses||0).toLocaleString('id-ID')}`;
        
        if (document.getElementById("sd-free-items")) document.getElementById("sd-free-items").innerText = (window.currentShiftData.totalFreeItems || 0) + " Item";
        if (document.getElementById("sd-discount-nom")) document.getElementById("sd-discount-nom").innerText = "Rp " + (window.currentShiftData.totalDiscountNominal || 0).toLocaleString('id-ID');
        
        document.getElementById("sd-net").innerText = `Rp ${(window.currentShiftData.netCash||0).toLocaleString('id-ID')}`; 
        document.getElementById("sd-food").innerHTML = foodHtml || "Tidak ada item terjual"; 
        document.getElementById("shift-detail-modal").classList.remove("hidden"); 
    };
};
...
window.viewOrderDetails = function(orderId) { 
    db.transaction(["orders"], "readonly").objectStore("orders").get(orderId).onsuccess = (e) => { 
        ...
        order.items.forEach(item => { 
            ...
            let applyQty = Math.min(remainingPromos.reduce((s,p)=> p.item===item.name? s+p.qty:s,0), item.qty); 
            if (applyQty > 0) { 
                let discountValue = applyQty * item.originalPrice; 
                itemsHtml += `<div style="display:flex; justify-content:space-between; font-size:13px; color:#e74c3c; margin-left:15px; border-bottom:1px dashed #eee; padding-bottom:4px;"><div>↘ Promo Hemat!</div><div>-Rp ${discountValue.toLocaleString('id-ID')}</div></div>`;
            } 
        }); 
    ...
    db.transaction(["orders"], "readonly").objectStore("orders").get(orderId).onsuccess = async (e) => { 
        ...
        if (order.customerPhone && order.customerPhone !== "-" && order.customerPhone !== "Walk-in" && !order.customerPhone.startsWith("999")) { 
            db.transaction(["members"], "readonly").objectStore("members").get(order.customerPhone).onsuccess = async (me) => { 
                ...
                await window.buildEscPosReceipt(order.orderId + " (COPY)", order, deposit, 0, order.paymentMethod, pts, fre); 
            };
        } else { 
            await window.buildEscPosReceipt(order.orderId + " (COPY)", order, deposit, 0, order.paymentMethod, 0, 0); 
        } 
    }; 
};
...
window.printShiftReportFromHistory = async function(shiftId) { 
    ...
    db.transaction(["local_shift_history"], "readonly").objectStore("local_shift_history").get(shiftId).onsuccess = async (e) => { 
        ...
        await window.buildShiftReportReceipt(shiftData); 
    }; 
};
...
window.openCashDrop = function() { document.getElementById("cashdrop-modal").classList.remove("hidden"); };
window.submitCashDrop = function() { 
    const admin = Number(document.getElementById("drop-admin").value) || 0; const bank = Number(document.getElementById("drop-bank").value) || 0; 
...
window.openCoinManagement = function() { document.getElementById("coin-management-modal").classList.remove("hidden"); };
...
window.runBackgroundSync = async function() { 
    ...
    try { 
        ...
        for (const log of ticketCoins) { 
            try { 
                let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: "syncTicketCoins", data: log }) });
                if ((await r.json()).status === "Success") { db.transaction(["ticket_coins"], "readwrite").objectStore("ticket_coins").delete(log.logId); } 
            } catch(e) {} 
        }
        ...
        for (const cr of coinRets) { 
            if (cr.syncStatus === "Pending") {
                try { 
                    let actionType = String(cr.notes).includes("Macet") ? "syncCoinJammed" : "syncCoinRetrieval";
                    let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: actionType, data: cr }) });
...
window.onload = async () => { 
    ...
    // Attach Dropdown Listener Dynamically to Inputs
    let elPhone = document.getElementById("cust-phone");
    let elName = document.getElementById("cust-name");
    if(elPhone) {
        elPhone.addEventListener("click", window.handleAutocomplete);
        elPhone.addEventListener("input", window.handleAutocomplete);
        elPhone.addEventListener("focus", window.handleAutocomplete);
    }
    if(elName) {
        elName.addEventListener("click", window.handleAutocomplete);
        elName.addEventListener("input", window.handleAutocomplete);
        elName.addEventListener("focus", window.handleAutocomplete);
    }
    ...
    window.setInterval(window.runBackgroundSync, 10000); 
    window.setInterval(function(){ window.syncMasterData(false); }, 30000); 
