...
function getDynamicSettings() { 
  return new Promise((resolve) => { 
    ...
    db.transaction(["settings"], "readonly").objectStore("settings").getAll().onsuccess = (e) => { 
      if (e.target.result) { 
        e.target.result.forEach(s => { settings[s.key] = s.value; }); 
      } 
      resolve(settings); 
    }; 
  }); 
  ...
  if (isMenuLocked) {
    ...
    if (lotteryBtn) {
      if (activeCustomerProfile && (activeCustomerProfile.lastClaimDate === todayStr || activeCustomerProfile.isNoWA)) {
        ...
        lotteryBtn.disabled = false; lotteryBtn.innerText = "🎫 Pilih Undian";
      } 
    }
...
function unlockMenu(isGuest) { 
  ...
  for (let i = 0; i < antreans.length; i++) { 
    ...
    if (!antreans[i].isLocked && phone !== "-" && otherPhone === phone) { 
      isDuplicate = true; 
      break;
    }
  } 
  ...
  if (!isGuest) { 
    db.transaction(["members"], "readonly").objectStore("members").get(phone).onsuccess = (e) => { 
      if (e.target.result) { 
        ...
        activeCustomerProfile = { phone: phone, name: name, points: 0, freeCoins: 0, spent: 0, storedRewards: {}, lastClaimDate: "", isNoWA: phone.startsWith("999") }; 
      } 
...
window.selectMember = function(phone) { 
  for (let i = 0; i < antreans.length; i++) { 
    ...
    if (!antreans[i].isLocked && otherPhone === phone) { 
      return alert("⚠️ Pelanggan ini sedang dilayani di Antrean " + (i+1) + ".");
    } 
  } 
  db.transaction(["members"], "readonly").objectStore("members").get(phone).onsuccess = (e) => {
    activeCustomerProfile = e.target.result;
...
function submitEditMember() { 
  ...
  db.transaction(["members"], "readonly").objectStore("members").get(oldPhone).onsuccess = (e) => { 
    ...
    antreans.forEach(a => { if (a.profile && a.profile.phone === oldPhone) { a.profile.phone = newPhone; a.phoneInput = newPhone; } });
...
async function syncMasterData() { 
  ...
  try { 
    ...
    if (result.status === "Success") {
      ...
      window.globalRecentShifts = result.data.recentShifts || [];
      ...
      staffStore.clear(); 
      result.data.staff.forEach(s => staffStore.add(s)); 
      const menuStore = tx.objectStore("menu");
      menuStore.clear();
      result.data.menu.forEach(m => menuStore.add(m)); 
      const memStore = tx.objectStore("members"); 
      memStore.clear(); 
      result.data.members.forEach(m => memStore.add(m)); 
...
function addToCart(item, qty) { 
  ...
  if (existing) { existing.qty += finalQty; } 
    ...
    currentCart.push({ ...item, qty: finalQty, originalPrice: item.price, expectedCoins: item.expectedCoins, hasMoq: item.hasMoq, moqQty: item.moqQty });
  } 
  renderCart(); 
} 
...
function renderCart() { 
  ...
  currentCart.forEach(item => { 
    ...
    total += lineTotal; 
    const qtyDisplay = item.qty % 1 !== 0 ? item.qty.toFixed(2) : item.qty;
    if (String(item.category).toLowerCase().includes('coin') || String(item.name).toLowerCase().includes('koin')) { cartCoins += item.qty; }
    container.innerHTML += `<div class="cart-item"><div><strong>${item.name}</strong><br><small style="color:#7f8c8d;">Rp ${item.price.toLocaleString('id-ID')} / unit</small></div><div style="display:flex; align-items:center;"><button class="cart-qty-btn" onclick="updateCartItemQty('${item.itemId}', -1)">-</button><span class="cart-qty">${qtyDisplay}</span><button class="cart-qty-btn" onclick="updateCartItemQty('${item.itemId}', 1)">+</button><div style="width:90px; text-align:right; font-weight:bold; margin-left:10px;">Rp ${lineTotal.toLocaleString('id-ID')}</div></div></div>`;
  }); 
  window.cartSubtotal = total; 
  document.getElementById("cart-grand-total").innerText = "Rp " + total.toLocaleString('id-ID'); 
  const payBtn = document.getElementById("btn-trigger-pay"); 
  if (payBtn) payBtn.disabled = currentCart.length === 0; 
  let promoHtml = ""; 
  if (activeCustomerProfile) {
    ...
    if (maxRedeemable > 0) { 
      promoHtml += `<div class="form-box"><label>🎁 Potongan Koin Gratis Pelanggan (Maks: ${maxRedeemable} koin)</label><input type="number" id="promo-loyalty" class="promo-input" min="0" max="${maxRedeemable}" value="0" data-type="loyalty" data-item="Koin" data-price="${activeCoinPrice}" oninput="applyPromo()"></div>`; 
    } 
    if (activeCustomerProfile.storedRewards) { 
      Object.entries(activeCustomerProfile.storedRewards).forEach(([item, qty]) => { 
        if (qty > 0) { 
          let matchedCartItem = currentCart.find(ci => ci.name === item || ci.subCategory === item || ci.category === item); 
          if (matchedCartItem) { 
            let maxClaimable = Math.min(qty, matchedCartItem.qty); 
            promoHtml += `<div class="form-box"><label>🎫 Klaim Undian Tersimpan: ${item} (Maks: ${maxClaimable})</label><input type="number" class="promo-input" min="0" max="${maxClaimable}" value="0" data-type="stored" data-item="${item}" data-price="${matchedCartItem.price}" oninput="applyPromo()"></div>`; 
          } 
        } 
      }); 
    } 
  } 
  document.getElementById("review-promo-section").innerHTML = promoHtml || "<p style='color:#7f8c8d; font-size:13px; margin:0;'>Tidak ada diskon / reward loyalty yang memenuhi syarat.</p>";
}
...
window.calculateRemaining = function() { 
  ...
  document.getElementById("review-remaining").innerText = `Rp ${(remaining >= 0 ? remaining : 0).toLocaleString('id-ID')}`; 
};
...
async function finalizeOrder(shouldPrint) { 
  ...
  const orderPayload = {
    ...
    customerName: custName, customerPhone: custPhone, orderStatus: status, items: currentCart, subtotal: window.cartSubtotal, discounts: free, grandTotal: window.cartGrandTotal, paymentMethod: payString, cashAmount: cash, qrisAmount: qris, transferAmount: transfer, hotelPiutangAmount: hotelPiutang, tamuPiutangAmount: tamuPiutang, freeAmount: free, remainingDue: 0, coinsEarned: coinsEarned, redeemedPromos: redeemedList, expectedCoins: expectedCoinsTotal, internalCoinsUsed: internalCoins, syncStatus: "Pending"
  }; 
  ...
  if (internalCoins > 0) {
    txMenu.openCursor().onsuccess = (ev) => { 
      ...
      if (cursor) { 
        if (String(cursor.value.name).toLowerCase() === "koin_fisik") { 
          const updated = cursor.value; 
          updated.currentStock = Math.max(0, updated.currentStock - internalCoins); 
  ...
  if (shouldPrint) { 
    await buildEscPosReceipt(orderPayload.orderId, orderPayload, (cash + qris + transfer + free + totalPiutang), 0, payString, newPoints, newFree);
  }
  closeReview(); 
  lockMenu(); 
  renderProductGrid(); 
...
window.viewOrderDetails = function(orderId) { 
  db.transaction(["orders"], "readonly").objectStore("orders").get(orderId).onsuccess = (e) => { 
    ...
    order.items.forEach(item => { 
      ...
      itemsHtml += `<div>• ${item.qty}x ${item.name} = Rp ${lineTotal.toLocaleString('id-ID')}</div>`;
    }); 
...
window.printShiftReportFromHistory = async function(shiftId) { 
  if (!btCharacteristic) return alert("Printer belum terhubung! Silakan hubungkan dari menu atas.");
  let s = window.globalRecentShifts ? window.globalRecentShifts.find(x => x.shiftId === shiftId) : null;
  if (s) {
    let temp = {...s};
    if (typeof temp.foodSummary === 'string') {
      let summaryObj = {};
      let lines = temp.foodSummary.split('\n');
      lines.forEach(line => {
        let match = line.match(/•\s*([\d.]+)[xx]\s*(.*)/);
        if (match) { summaryObj[match[2].trim()] = parseFloat(match[1]); }
      });
      temp.foodSummary = summaryObj;
    }
    await buildShiftReportReceipt(temp);
  } else {
...
async function printCurrentShiftReport() { 
  ...
  const tempPayload = { shiftId: currentShiftId, cashier: currentCashier, loginTime: currentLoginTime, logoutTime: new Date().toISOString(), totalCustomers: data.totalCustomers, totalOrders: data.totalOrders, totalOmset: data.totalOmset, totalCash: data.totalCash, totalQris: data.totalQris, totalTransfer: data.totalTransfer, totalHotelPiutang: data.totalHotelPiutang, totalTamuPiutang: data.totalTamuPiutang, totalFree: data.totalFree, totalExpenses: data.totalExpenses, netCash: data.net, foodSummary: data.foodSummary, meterToken: meterT, meterPasca: meterP }; 
  try { await buildShiftReportReceipt(tempPayload); alert("Laporan berhasil dikirim ke printer!"); } 
  catch(e) { alert("Gagal mencetak: " + e.toString()); } 
} 

function renderActiveTickets() { 
  const grid = document.getElementById("ticket-grid-container"); 
  grid.innerHTML = "";
  activeLaundryTickets.forEach((ticket) => { 
    ...
    grid.innerHTML += `<div class="ticket-card ${isReady ? 'ready' : ''}"><div class="ticket-header"><span>${ticket.orderId}</span><span>${ticket.customerName}</span></div><div>${receiptText}</div>${buttonsHtml}</div>`;
  });
}
...
window.viewShiftDetails = function(shiftId) { 
  ...
  if (s) {
    ...
    db.transaction(["local_shift_history"], "readonly").objectStore("local_shift_history").get(shiftId).onsuccess = (e) => {
      let localS = e.target.result;
      if (!localS) return alert("Data tidak ditemukan."); 
      showModal(localS);
    };
  }
  function showModal(s) {
    ...
    if(s.foodSummary) { 
      if (typeof s.foodSummary === 'string') {
        foodHtml = s.foodSummary.replace(/\n/g, '<br>');
      } else {
...
function renderHistoryList(type) { 
  ...
  if (type === 'orders') { 
    ...
    if (window.globalRecentShifts && window.globalRecentShifts.length > 0) {
      shifts = window.globalRecentShifts.filter(s => s.cashier === currentCashier).reverse();
    }
    if (shifts.length === 0) {
      db.transaction(["local_shift_history"], "readonly").objectStore("local_shift_history").getAll().onsuccess = (e) => { 
        const localShifts = e.target.result.filter(s => s.cashier === currentCashier).reverse();
        displayShifts(localShifts.slice(0, 6));
      }; 
    } else {
      displayShifts(shifts.slice(0, 6));
    }

    function displayShifts(shiftList) {
      if(shiftList.length === 0) return container.innerHTML = `<div style="padding:20px; text-align:center;">Belum ada histori shift Anda.</div>`;
      shiftList.forEach(s => { 
...
function checkExpiredShifts() { 
  ...
  db.transaction(["active_shifts"], "readonly").objectStore("active_shifts").getAll().onsuccess = (e) => { 
    ...
    activeShifts.forEach(shift => { 
      ...
      if (now - referenceTime > 4 * 60 * 60 * 1000) { 
...
function performAutoClose(shift) { 
  let tCust = 0; let tOrders = 0; let tOmset = 0; let tCash = 0;
  let tQris = 0; let tTransfer = 0; let hPiu = 0; let tPiu = 0; let tFree = 0;
  let tExpense = 0; let foodSummary = {};
  db.transaction(["orders", "expenses"], "readonly").objectStore("orders").getAll().onsuccess = (e) => { 
    ...
    validOrders.forEach(o => { 
      tOrders++; if(o.customerPhone && o.customerPhone !== "-") tCust++; tOmset += o.grandTotal; tCash += (o.cashAmount || 0); tQris += (o.qrisAmount || 0); tTransfer += (o.transferAmount || 0); hPiu += (o.hotelPiutangAmount || 0); tPiu += (o.tamuPiutangAmount || 0); tFree += (o.freeAmount || 0); 
    ...
    db.transaction(["expenses"], "readonly").objectStore("expenses").getAll().onsuccess = (ex) => { 
      ...
      const shiftPayload = {
        ...
        totalCustomers: tCust, totalOrders: tOrders, totalOmset: tOmset, totalCash: tCash, totalQris: tQris, totalTransfer: tTransfer, totalHotelPiutang: hPiu, totalTamuPiutang: tPiu, totalFree: tFree, totalExpenses: tExpense, netCash: netCash, foodSummary: foodSummary, meterToken: 0, meterPasca: 0, closeNote: "System Auto-Closed (>4h Idle Expired)", syncStatus: "Pending"
      };
      ...
      if (!window.enableDrawerTracking) { 
        txW.objectStore("cash_drops").add({ dropId: "DRP-" + Date.now() + Math.floor(Math.random()*100), timestamp: new Date().toISOString(), cashier: shift.cashierName || "System", shiftId: shift.shiftId, toAdmin: netCash, toBank: 0, leftInDrawer: 0, notes: "[Ke Admin] Auto-Close Shift > 4 Jam Idle", syncStatus: "Pending" });
      } 
  ...
  try { 
      ...
      if (order.syncStatus === "Pending") { 
        ...
        try { 
          ...
          if ((await r.json()).status === "Success") { 
            ...
            order.syncStatus = "Pending"; db.transaction(["orders"], "readwrite").objectStore("orders").put(order);
          } 
    ...
    for (const drop of drops) { 
      if (drop.syncStatus === "Pending") { 
        try { 
          let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: "syncCashDrop", data: drop }) });
    ...
    for (const report of reports) { 
      if (report.syncStatus === "Pending") { 
        try { 
          let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: "syncShiftReport", data: report }) });
    ...
    for (const exp of expenses) { 
      if (exp.syncStatus === "Pending") { 
        try { 
          let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: "syncExpense", data: exp }) });
    ...
    for (const req of voids) { 
      try { 
        ...
        let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: actionType, ...payload }) });
    ...
    for (const mem of members) { 
      try { 
        let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: "syncMember", data: mem }) });
    ...
    for (const upd of phoneUpds) { 
      try { 
        let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: "updateMemberPhone", data: upd }) });
        if ((await r.json()).status === "Success") { db.transaction(["phone_updates"], "readwrite").objectStore("phone_updates").delete(upd.id); } 
      } catch(e) {} 
    } 
    ...
    for (const ret of coinRets) { 
      if (ret.syncStatus === "Pending") { 
        ...
        try { 
          let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: actionCode, data: ret }) });
          if ((await r.json()).status === "Success") { ret.syncStatus = "Synced"; db.transaction(["coin_retrievals"], "readwrite").objectStore("coin_retrievals").put(ret); } 
        } catch(e) {} 
      } 
    } 
    ...
    for (const tc of ticketCoins) { 
      if (tc.syncStatus === "Pending") { 
        try { 
          let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: "syncTicketCoins", data: tc }) });
          if ((await r.json()).status === "Success") { tc.syncStatus = "Synced"; db.transaction(["ticket_coins"], "readwrite").objectStore("ticket_coins").put(tc); } 
        } catch(e) {} 
      } 
    } 
    ...
    for (const claim of promoClaims) { 
      if (claim.syncStatus === "Pending") { 
        try { 
          let r = await fetch(API_URL, { method: 'POST', mode: 'cors', redirect: 'follow', body: JSON.stringify({ action: "syncPromoClaim", data: claim }) });
...
// ==========================================
// GOOGLE APPS SCRIPT BACKEND ENGINE FUNCTIONS
// ==========================================

function doGet(e) { 
  try { 
    const db = SpreadsheetApp.getActiveSpreadsheet(); 
    let masterDrawerBalance = 0; 
    let loyaltyTarget = 10; 
    const settingsData = db.getSheetByName("Settings").getDataRange().getValues();
    let settingsCache = {}; 
    for (let i = 1; i < settingsData.length; i++) { 
      if (!String(settingsData[i][0]).trim()) continue; 
      let val = settingsData[i][1];
      if (settingsData[i][0] === "Master_PIN") { val = computeSHA256(val); } 
      settingsCache[settingsData[i][0]] = val;
      if (settingsData[i][0] === "Saldo_Laci") masterDrawerBalance = Number(settingsData[i][1]) || 0; 
      if (settingsData[i][0] === "Loyalty_Target") loyaltyTarget = Number(settingsData[i][1]) || 10;
    } 
    let promoCache = []; 
    const promoSheet = db.getSheetByName("Promo_Master"); 
    if (promoSheet) { 
      const pVals = promoSheet.getDataRange().getValues();
      for(let i=1; i<pVals.length; i++) { 
        let promoCode = String(pVals[i][0]).trim(); 
        if (!promoCode) continue;
        promoCache.push({ code: promoCode.toUpperCase(), rewardItem: pVals[i][1], rewardQty: Number(pVals[i][2]), weeklyQuota: Number(pVals[i][3]), usedQuota: Number(pVals[i][4]) }); 
      } 
    } 
    const liveStockBalances = calculateMasterStock(db); 
    let koinFisikStock = 0;
    const menuDisplayData = db.getSheetByName("Menu_Master").getDataRange().getDisplayValues(); 
    for (let i = 1; i < menuDisplayData.length; i++) { 
      if (String(menuDisplayData[i][3]).trim() === "Koin_Fisik") { 
        koinFisikStock = (liveStockBalances["Koin_Fisik"] !== undefined) ? liveStockBalances["Koin_Fisik"] : Number(menuDisplayData[i][6]); break; 
      } 
    }
    let menuCache = []; 
    for (let i = 1; i < menuDisplayData.length; i++) { 
      let itemIdStr = String(menuDisplayData[i][0]).trim(); 
      if (!itemIdStr) continue;
      let isActive = "TRUE"; 
      if (menuDisplayData[i].length > 13 && String(menuDisplayData[i][13]).trim() !== "") { isActive = String(menuDisplayData[i][13]).toUpperCase().trim(); } 
      if (isActive === "FALSE") continue;
      let itemName = String(menuDisplayData[i][3]).trim(); 
      if (itemName === "Koin_Fisik") continue;
      let isTracked = String(menuDisplayData[i][5]).toUpperCase() === "TRUE"; 
      let currentStockLevel = 0; 
      if (itemName.toLowerCase().includes("koin")) { currentStockLevel = isTracked ? koinFisikStock : 0; } 
      else { currentStockLevel = (isTracked && liveStockBalances[itemName] !== undefined) ? liveStockBalances[itemName] : Number(menuDisplayData[i][6]); } 
      menuCache.push({
        itemId: itemIdStr, category: menuDisplayData[i][1], subCategory: menuDisplayData[i][2] || "Other", name: itemName,
        price: Number(String(menuDisplayData[i][4]).replace(/[^\d.-]/g, '')), trackStock: isTracked, currentStock: currentStockLevel, inputMode: String(menuDisplayData[i][7]).toUpperCase(), workflow: String(menuDisplayData[i][8]).toUpperCase(), expectedCoins: Number(menuDisplayData[i][10]) || 0, hasMoq: String(menuDisplayData[i][11]).trim().toUpperCase() === "TRUE", moqQty: Number(menuDisplayData[i][12]) || 1
      });
    } 
    ...
    if (txSheet) { 
      ...
      const startRow = Math.max(1, txData.length - 300);
      for (let i = startRow; i < txData.length; i++) { 
        let status = String(txData[i][5]).trim();
        if (status === "Processing" || status === "Ready for Pickup") { 
          activeLaundryOrders.push({ orderId: txData[i][0], timestamp: txData[i][1], customerName: txData[i][3], customerPhone: txData[i][4], orderStatus: status, readableReceipt: txData[i][7], grandTotal: Number(txData[i][10]), cashAmount: Number(txData[i][12]), qrisAmount: Number(txData[i][13]), transferAmount: Number(txData[i][14]) || 0, hotelPiutangAmount: Number(txData[i][15]) || 0, tamuPiutangAmount: Number(txData[i][16]) || 0, freeAmount: Number(txData[i][17]) || 0, expectedCoins: Number(txData[i][21]) || 0 }); 
        } 
      } 
    } 
    let uniqueExpCats = new Set(); 
    const expSheet = db.getSheetByName("Expenses"); 
    if (expSheet) { 
      const expData = expSheet.getDataRange().getValues();
      for(let i=1; i<expData.length; i++) { let catStr = String(expData[i][3]).trim(); if(catStr) uniqueExpCats.add(catStr); } 
    } 
    let latestClaims = {}; 
    const logSheet = db.getSheetByName("Promo_Claim_Log");
    if (logSheet) { 
      const claimLogData = logSheet.getDataRange().getDisplayValues();
      for (let i = 1; i < claimLogData.length; i++) { 
        let phone = normalizePhone(claimLogData[i][2]);
        let ts = String(claimLogData[i][1]); 
        if (ts.length >= 10) latestClaims[phone] = ts.substring(0, 10); 
      } 
    } 
    const membersData = db.getSheetByName("Members").getDataRange().getDisplayValues(); 
    let membersCache = [];
    for (let i = 1; i < membersData.length; i++) { 
      let phoneStr = String(membersData[i][0]).trim(); 
      if (!phoneStr) continue;
      if (phoneStr && !phoneStr.startsWith('0') && phoneStr.length > 8) phoneStr = '0' + phoneStr; 
      let targetPhone = normalizePhone(phoneStr);
      let sRewards = {}; 
      try { sRewards = JSON.parse(String(membersData[i][6] || "{}")); } catch(e) {} 
      let lastClaim = latestClaims[targetPhone] || "";
      membersCache.push({ phone: phoneStr, name: membersData[i][1], spent: Number(String(membersData[i][2]).replace(/[^\d.-]/g, ''))||0, points: Number(membersData[i][3])||0, freeCoins: Number(membersData[i][4])||0, storedRewards: sRewards, lastClaimDate: lastClaim }); 
    } 
    const staffData = db.getSheetByName("Staff").getDataRange().getValues();
    let staffCache = [];
    for (let i = 1; i < staffData.length; i++) { 
      let pinVal = String(staffData[i][0]).trim(); 
      if (!pinVal) continue;
      staffCache.push({ pin: computeSHA256(pinVal), name: staffData[i][1], role: staffData[i][2] }); 
    } 
    let authStatuses = { orders: {}, expenses: {} };
    if (txSheet) { 
      const txData = txSheet.getDataRange().getValues(); 
      const startTx = Math.max(1, txData.length - 200);
      for (let i = startTx; i < txData.length; i++) { 
        let status = String(txData[i][5]).trim(); 
        let auth = String(txData[i][6]).toLowerCase().trim();
        if (auth === 'ok' || auth === 'approved' || auth === 'ya') { if (status !== 'Voided') { txSheet.getRange(i + 1, 6).setValue("Voided"); status = 'Voided'; } } 
        else if (auth === 'no' || auth === 'declined' || auth === 'tolak' || auth === 'reject') { if (status === 'Void Pending') { txSheet.getRange(i + 1, 6).setValue("Completed"); status = 'Completed'; } } 
        authStatuses.orders[txData[i][0]] = { status: status, auth: auth }; 
      }
    } 
    if (expSheet) { 
      const expData = expSheet.getDataRange().getValues(); 
      const startExp = Math.max(1, expData.length - 200);
      for (let i = startExp; i < expData.length; i++) { 
        let status = String(expData[i][6]).trim(); 
        let auth = String(expData[i][7]).toLowerCase().trim();
        if (auth === 'ok' || auth === 'approved' || auth === 'ya') { if (status !== 'Voided') { expSheet.getRange(i + 1, 7).setValue("Voided"); status = 'Voided'; } } 
        else if (auth === 'no' || auth === 'declined' || auth === 'tolak' || auth === 'reject') { if (status === 'Void Pending') { expSheet.getRange(i + 1, 7).setValue("Active"); status = 'Active'; } } 
        authStatuses.expenses[expData[i][0]] = { status: status, auth: auth }; 
      }
    } 
    const shiftSheet = db.getSheetByName("Shift_Reports");
    let recentShiftsCache = [];
    if (shiftSheet) {
      const shiftData = shiftSheet.getDataRange().getValues();
      const startShiftRow = Math.max(1, shiftData.length - 40);
      for (let i = startShiftRow; i < shiftData.length; i++) {
        if (!shiftData[i][0]) continue;
        recentShiftsCache.push({
          shiftId: String(shiftData[i][0]), cashier: String(shiftData[i][1]), loginTime: String(shiftData[i][2]), logoutTime: String(shiftData[i][3]),
          totalCustomers: Number(shiftData[i][4]) || 0, totalOrders: Number(shiftData[i][5]) || 0, totalOmset: Number(shiftData[i][6]) || 0,
          totalCash: Number(shiftData[i][7]) || 0, totalQris: Number(shiftData[i][8]) || 0, totalTransfer: Number(shiftData[i][9]) || 0,
          totalHotelPiutang: Number(shiftData[i][10]) || 0, totalTamuPiutang: Number(shiftData[i][11]) || 0, totalFree: Number(shiftData[i][12]) || 0,
          totalExpenses: Number(shiftData[i][13]) || 0, netCash: Number(shiftData[i][14]) || 0, foodSummary: String(shiftData[i][15] || "")
        });
      }
    }
    const responsePayload = { 
      status: "Success", timestamp: new Date().toISOString(), masterDrawerBalance: masterDrawerBalance, 
      data: { settings: settingsCache, menu: menuCache, members: membersCache, staff: staffCache, authStatuses: authStatuses, activeLaundryOrders: activeLaundryOrders, promos: promoCache, loyaltyTarget: loyaltyTarget, expenseCategories: Array.from(uniqueExpCats), recentShifts: recentShiftsCache } 
    };
