...
window.attemptLogin = async function() {
    ...
    try {
        ...
        // Jika belum ada di lokal, paksa narik dari Cloud (Online Sync) sebelum ditolak
        if (!staff) {
            if (navigator.onLine) {
                if(loginBtn) loginBtn.innerText = "Menarik Data...";
                await window.syncMasterData(true); // Await strictly
...
// FIX MUTLAK 1: ONMOUSEDOWN UNTUK MENCEGAH DROPDOWN HILANG SEBELUM TEREKSEKUSI
window.handleAutocomplete = function(e) {
    ...
    db.transaction(["members"], "readonly").objectStore("members").getAll().onsuccess = (ev) => {
        ...
        // Menggunakan onmousedown karena klik biasa kalah cepat dengan fungsi close dari outer document click
        if (matches.length > 0) {
...
// FIX 2: MENGAMANKAN TOMBOL BATAL CHECKOUT
...
window.syncMasterData = async function(forceAwait = false) {
    ...
    try {
        ...
        if (result.status === "Success") {
            ...
            if(forceAwait) await p1; // Only block if explicitly requested (like when login fails)
