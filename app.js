...
window.unlockMenu = function(isGuest) {
    ...
    if (isGuest) { 
        ...
        // FIX: Cegah profil staff tertimpa menjadi member reguler baru
        if (activeCustomerProfile && (activeCustomerProfile.isStaffProfile || phone.toUpperCase().startsWith("STF"))) {
            proceedToUnlock(phone, name);
            return;
        }
...
window.openReview = async function() {
    ...
    // Deteksi otomatis via nomor HP "STF-" atau isStaffProfile
    let isStaffProfile = activeCustomerProfile && (activeCustomerProfile.isStaffProfile || String(activeCustomerProfile.phone).toUpperCase().startsWith("STF"));
    ...
    if (isStaffProfile) {
        ...
        let selectedStaffObj = allStaffList.find(s => s.name.toLowerCase().trim() === selectedStaffName.toLowerCase().trim());
        staffFreeCoins = selectedStaffObj ? (Number(selectedStaffObj.freeCoins) || 0) : 0;
    }
    
    // HANYA 1 KOTAK: Muncul jika profil adalah Staff dan punya koin
    ...
    if (activeCustomerProfile) {
        ...
        // SEMBUNYIKAN KOTAK LOYALTY JIKA PELANGGAN ADALAH STAFF
        if (maxRedeemable > 0 && !isStaffProfile) {
...
window.openShiftReport = function(historyData = null) {
    ...
    if (historyData) {
        ...
        tx.oncomplete = async () => {
            ...
            let freeItemsSummary = {}; // <--- TAMBAHKAN INI
